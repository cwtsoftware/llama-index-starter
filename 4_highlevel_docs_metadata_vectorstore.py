...
# definira se indeks
# tijekom ovog koraka VectorStoreIndex pretvara dokumente u numeričke zapise u duljinama po 1024 tokena (default postavka) (1 token je cca 4 znaka)
# i onda kreira objekt VectorStoreIndex s kojime se nadalje radi.
# generalno u praksi će postojati dedicirana vektorska baza podataka poput Chroma ili Pinecone iz kojih će se učitati indeks
# kao što je već bilo prikazano u prijašnjim modulima, tijekom ovog tutoriala će se kreirani numerički zapisi spremiti lokalno
# prema default postavkama koristi se openai za pretvaranje u numeričke zapise 
index = VectorStoreIndex.from_documents(
    documents
)

# kao što je već spomenuto tijekom faze upita se upit pretvara u numerički zapis te se prema tome traže
# najsličniji čvorovi. Prema default postavkama uzimaju se 2 najsličnija čvora i oni 
# se postavljaju kao kontekst te se nadoda postavljen upit i to dvoje se šalje llm-u
# ponekad je dobro i povećati broj najsličnijh čvorova koji se proslijeđuju kao kontekst
# similarity_top_k atribut se koristi za definiranje broja rezultata koji se proslijeđuju
